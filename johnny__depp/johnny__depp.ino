 ...
 if (millis() > timer) 
  {
    ...
    if (distance <= maxDistance && lineCount >= 4){ //Condition that it counted 4 lines before to avoid conflict with the start code.
          // Anything closer than 20CM it will avoid it
          Serial.println("left");
          Serial.println(" ");
          driveLeft(backwardsSpeedLeft, rightSlowSpeed + speedTurns + additionalSpeed);
          delay(700);
...
void finishLights() {  // Cyan
  setPixelColor(0, 0, 255, 255); //left back
  setPixelColor(1, 0, 255, 255); //right back
  setPixelColor(2, 0, 255, 255); //right front
  setPixelColor(3, 0, 255, 255); //left front
}
void leftLights() {  // Green but purple should appear on the left front LED
  setPixelColor(0, 102, 205, 170); //left back
  setPixelColor(1, 102, 205, 170); //right back
  setPixelColor(2, 102, 205, 170); //right front
  setPixelColor(3, 138, 43, 226); //left front
}
void rightLights() { // Green but purple should appear on the right front LED
  setPixelColor(0, 102, 205, 170); //left back
  setPixelColor(1, 102, 205, 170); //right back
  setPixelColor(2, 138, 43, 226); //right front
  setPixelColor(3, 102, 205, 170); //left front
}
void neutraltLights() { // Green
  setPixelColor(0, 102, 205, 170); //left back
  setPixelColor(1, 102, 205, 170); //right back
  setPixelColor(2, 102, 205, 170); //right front
  setPixelColor(3, 102, 205, 170); //left front
